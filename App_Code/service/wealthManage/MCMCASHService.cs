...
/// </summary>
public class MCMCASHService : System.Web.UI.Page
{
    //引用時不用再NEW
    private static MCMCASHService _instance = new MCMCASHService();
    public static MCMCASHService Instance
    {
        get
        {
            return _instance;
    ...
    /// <returns></returns>
    ///  //20220713進度
        //傳入U_MDEP,U_NUM,COMID 3個參數
    public List<VMCASHDataEntity> getMCMCASHData(string Pay_Tag, string Done_Tag, string keyword)
    {
        //20220713進度
        //傳入U_MDEP,U_NUM,COMID 3個參數
        ...
        //var U_Num = HttpContext.Current.Session["U_Num"];

        //string IP = Session["IP"] == null ? "" : Session["IP"].ToString();//20220712
        //var test= Session["U_Num"].ToString();//不可執行
        List<VMCASHDataEntity> vdeList = VMCASHhgDao.Instance.getVMCASHData(Pay_Tag, Done_Tag, keyword);//<==========
        //string test=sess
        if (vdeList != null && vdeList.Count != 0)
        {
            //if (!common.isEmpty(vde.M_id))
            //{
    ...
    public string  delMCMCASHData(string ca_id)
    {
        ...
        //string IP = Session["IP"] == null ? "" : Session["IP"].ToString();//20220712
        //var test= Session["U_Num"].ToString();//不可執行
        string result = VMCASHhgDao.Instance.delVMCASHData(ca_id);//<==========
        //string test=sess
