...
public class customerManageHandler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        switch (action)
        {
            //case "searchMCMCASHData" :
            //    //, gcp = new getCustormerParams();
            ...
            //    context.Response.Write(VMCASHDataEntity());
            //    break;
            //20220712                getMCMCASHData
            case "searchMCMCASHData":
                string ComID = context.Request["ComID"];
                string Pay_Tag = context.Request["Pay_Status"];//付款狀態
                string Done_Tag = context.Request["Done_Tag"];//案件狀態
                string keyword = context.Request["keyword"];//關鍵字

                context.Response.Write(getMCMCASHData(Pay_Tag, Done_Tag, keyword));
                break;

            case "deleteMCMCASHData"://deleteMCMCASHData
                string ca_id = context.Request["ca_id"];


                context.Response.Write(delMCMCASHData(ca_id));
                break;
            ...
            case "deleteCustomerData":
                M_ID = context.Request["M_ID"];
                context.Response.Write(deleteCustomerData(M_ID));
                break;
            case "getAppointData":
                M_ID = context.Request["M_ID"];
            ...
            case "getAnalystForCashFlow":
                ...
                context.Response.Write(getAnalystForCashFlow(ComID_));
                break;
            case "getAnalystItem":
    ...
    /// </summary>
    /// <returns></returns>                                    
    public string getMCMCASHData(string Pay_Tag, string Done_Tag, string keyword)
    {
        string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
        //log.startWrite("'VDE' :"+VDE.ToString(), className, methodName);
        VMCASHDataEntity vde = new VMCASHDataEntity();
        List<VMCASHDataEntity> vdeList = MCMCASHService.Instance.getMCMCASHData(Pay_Tag, Done_Tag, keyword);//<==========
    ...
    /// </summary>
    /// <returns></returns>                                    
    public string delMCMCASHData(string ca_id)
    {
        string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
        //log.startWrite("'VDE' :"+VDE.ToString(), className, methodName);
        VMCASHDataEntity vde = new VMCASHDataEntity();
        string result = MCMCASHService.Instance.delMCMCASHData(ca_id);//<==========

        log.endWrite(ca_id.ToString(), className, methodName);
    ...
    /// </summary>
    /// <returns></returns>
    //public string getMCMCASHDataData(VMCASHDataEntity VDE)
    //{
    //    string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
    //    log.startWrite("'VDE' :" + VDE.ToString(), className, methodName);

    //    List<VMCASHDataEntity> cdeList = MCMCASHService.Instance.getMCMCASHData(VDE);//<==========
