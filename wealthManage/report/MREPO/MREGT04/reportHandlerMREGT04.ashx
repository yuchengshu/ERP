...
public class reportHandlerMREGT01_List : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string Select_Employ = string.Empty;

        switch (action)
        {
            //已休年假表
            case "getMREGT04DataList":
                StartDate = context.Request["StartDate"];
                EndDate = context.Request["EndDate"];
                Select_Employ = context.Request["Select_Employ"];
                Select_Dept = context.Request["Select_Dept"];

                context.Response.Write(getMREGT04DataList(StartDate,EndDate,Select_Employ,Select_Dept));
                break;
            //出缺勤異常表
    ...
    //取得已休年假表20230111WORK
    public string getMREGT04DataList(string StartDate,string EndDate,string Select_Employ,string Select_Dept)
    {
        DateTime StartDateDt = DateTime.Parse(StartDate);
        DateTime EndDateDt = DateTime.Parse(EndDate);
        string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
        List<MREGT04Entity> MREGT04DatalList = new List<MREGT04Entity>();
        MREGT04DatalList = reportServiceMREGT04.Instance.getMREGT04List(Select_Employ,Select_Dept,StartDate,EndDate);
        return Newtonsoft.Json.JsonConvert.SerializeObject(MREGT04DatalList);
    }
