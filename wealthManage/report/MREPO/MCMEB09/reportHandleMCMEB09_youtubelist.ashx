<%@ WebHandler Language="C#" Class="reportHandleMCMEB09_youtubelist" %>
...
public class reportHandleMCMEB09_youtubelist : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        switch (action)
        {
            ...
            case "getMCMEB09_youtubelistData":
                P_ID = context.Request["P_ID"];//分析師編號

                MR_DateY = context.Request["MR_DateY"];
                MR_DateM = context.Request["MR_DateM"];
                ///20221110進度(取回對應的資料,並顯示在畫面上)
                context.Response.Write(getMCMEB09_youtubelistData(P_ID, MR_DateY, MR_DateM));
                break;
    ...
    //20221110進度
    public string getMCMEB09_youtubelistData(string P_ID, string MR_DateY, string MR_DateM)
    {
        string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
        List<ClickDetailEntity> getMCMEB09_youtubelist_Click = new List<ClickDetailEntity>();
        //log.startWrite(" 'MR_DateY' : " + MR_DateY + " 'MR_DateM' : " + MR_DateM + " 'MP' : " + MP, className, methodName);
        if (P_ID != "")
        {
            string T_ID = "";
            //加入由分析師編號取得T_ID
            string GetTIDfromU_Num = "select t_id from WAnalysts where U_Num='" + P_ID + "'";
            ...
            getMCMEB09_youtubelist_Click = reportServiceMCMEB09.Instance.getClickDetail("video", T_ID, MR_DateY, MR_DateM);//<==========
            log.endWrite("'getMCMEB09_youtubelist_Click' : " + getMCMEB09_youtubelist_Click.ToString(), className, methodName);
            return Newtonsoft.Json.JsonConvert.SerializeObject(getMCMEB09_youtubelist_Click);
        }
        log.endWrite("'mcseList' : " + getMCMEB09_youtubelist_Click.ToString(), className, methodName);
        return Newtonsoft.Json.JsonConvert.SerializeObject(getMCMEB09_youtubelist_Click);
    }
