<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="MCMEB09_critiquelist.aspx.cs" Inherits="wealthManage_report_MREPO_MCMEB09_MCMEB09_critiquelist" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
                ...
                <div class="col-md-12">
                    <div class="box box-danger">
                        <div class="box-header">
                            ...
                            <h1 class="box-title">文章-點閱細表</b> </h1>
    ...
    <script>
        var teststring = "<%=TestString %>";/*從cs端取得值(分析師編號) */
        var C_ID = "<%=C_ID %>";/*從cs端取得值(分析師編號) */
        //var C_ID = "<%=C_ID %>";/*從cs端取得值(分析師編號) */
        var MR_DateY ="<%=MR_DateY %>";/*從cs端取得值 */
        var MR_DateM = "<%=MR_DateM %>";/*從cs端取得值 */
       <%-- var C_ID2 ="<%=C_ID2 %>";/*從cs端取得值(分析師編號) */
        var MR_DateY2 ="<%=MR_DateY2 %>";/*從cs端取得值 */
        var MR_DateM2 = "<%=MR_DateM2 %>";/*從cs端取得值 */--%>
        ...
        var getTotalPage = function (count) {
            ...
            return pageCount;            //C:\Work\WebSites\ERP\wealthManage\report\MREPO\MCMEB09
        };                                                                       //reportHandleMCMEB09_critiquelist.ashx
        var MCMEB09_critiquelistHandlerUrl_G = "/wealthManage/report/MREPO/MCMEB09/reportHandleMCMEB09_critiquelist.ashx";
        var makeHtml = function () {
            //alert('!!');
            $("#critiquelistData").empty();
            //alert('!!!!!');
            //MCMEB09_youtubelistDataJson_G
            //MCMEB09_youtubelistDataJson_G
            if (MCMEB09_critiquelistDataJson_G.length != 0) {
                var sb = new string_M.StringBuilder();
                var n = 0;
                var startCount = (NowPage_G - 1) * 30 + 1;
                var endCount = 30 * NowPage_G;
                var isMember = false;
                    //MCMEB09_medialistHandlerUrl_G
                $.each(MCMEB09_critiquelistDataJson_G, function (i, v) {
                    n += 1;
                    if (n >= startCount && n <= endCount) {
                        ...
                        //https://www.hwa-guan.com.tw/Wealth/ViewPoint/RadioDetail
                        sb.append("<a href='https://www.hwa-guan.com.tw/Wealth/ViewPoint/ArticleContent?C_id=");
                        sb.append(v.V_id);
                        sb.append("' target=");
                        sb.append(v.V_id);
                        sb.append(">")
                ...
                append_M("#critiquelistData", sb.toString());
        ...
        var startThisPage = function () {
            var data = "C_ID=" + C_ID + "&MR_DateY=" + MR_DateY + "&MR_DateM=" + MR_DateM + "&action=getMCMEB09_critiquelistData";
            var searchCustomerDataSuccess = function (back) {
                MCMEB09_critiquelistDataJson_G = jsonParse_M(back);
            ...
            var error = function (e) { Log_M("MCMEB09_critiquelistData error=" + e); unBlockUI_M(); };
            //alert('Call CustomerManageHandlerUrl_G2');
            ajax_M(MCMEB09_critiquelistHandlerUrl_G, data, searchCustomerDataSuccess, error);//<===
        }
        //MCMEB09_critiquelistDataJson_G
        var MCMEB09_critiquelistData = function () {
            var data = "C_ID=" + C_ID + "&MR_DateY=" + MR_DateY + "&MR_DateM=" + MR_DateM + "action = getMCMEB09_critiquelistData";
            var success = function (back) {
                var json = jsonParse_M(back);
                MCMEB09_critiquelistData = jsonParse_M(back);
                //$muser_MenuSetupPageList.Table.makeTbody("#MUSER_MenuSetupData", json, 10, "#pageBar", deleteData, getMid);
                makeHtml();//依所得資料產生畫面
            };
            var error = function (e) { Log_M("MCMEB09_critiquelistData error=" + e); unBlockUI_M(); };
            ajax_M(MCMEB09_critiquelistHandlerUrl_G, data, success, error);
        };
        //20221109
        $(function () {
            //alert('Start!!');
