...
public class MREPO_queryHandler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        string U_Name = "";
        string T_ID = context.Request["T_ID"];//要修改的員工編號
        ...
        {//updateMREPO_queryData
         //取得[待開發票作業-線上]資料
         ////取得待開發票作業-線上(20221019進度)
         //
            case "UpdateInvoiceNumber":
                var InvoiceNumber = context.Request.Form["InvoiceNumber"];
                context.Response.Write(UpdateInvoiceNumber(T_ID, InvoiceNumber));///取得待開發票作業-線上(20221019進度)
                break;

            case "getOINVO_qyery_01DataList":
                ...
                context.Response.Write(getOINVO_qyery_01DataList());///取得待開發票作業-線上(20221019進度)
                break;

            case "GetItemByT_ID":
                context.Response.Write(GetItemByT_ID(T_ID));
                break;
    ...
    public string UpdateInvoiceNumber(string T_ID, string InvoiceNumber)
    {
        string connectionStrings = "sqlConnectHG_Test_TEST";
        string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
        using (SqlConnection conn = new SqlConnection(WebConfigurationManager.ConnectionStrings[connectionStrings].ToString()))
        {
            SqlConnection connection = conn;
            string commandText = "UPDATE [dbo].[NEW_VINVO] SET [Inv_num]='" + InvoiceNumber + "' Where T_ID='" + T_ID + "'";
    ...
    //20221026(GetItemByT_ID)
    public string GetItemByT_ID(string _T_ID)
    {
        ERPInvoice eRPInvoice = new ERPInvoice();
        string connectionStrings = "sqlConnectHG_Test_TEST";
        using (SqlConnection conn = new SqlConnection(WebConfigurationManager.ConnectionStrings[connectionStrings].ToString()))
        {
            //string commandText = "SELECT MT_COM,a.INV_SendState,a.T_ID,a.M_ID,a.INV_Item,a.INV_UnifyNum,b.M_Name,a.INV_ComName," +
            //                    "b.H_Address,b.HTown,b.HZipCode,b.H_Address,b.HCity,b.MTel1,b.EMail,b.INV_CarrierNum,b.T_PayMoney,b.M_Name" +
            //                    " FROM [HG].[dbo].[MTRAT] a left join [HG].[dbo].[VINVO] b  on a.T_ID=b.T_ID Where a.T_ID='" + _T_ID + "'";
            string commandText = "SELECT MT_COM,a.INV_SendState,a.T_ProductTag,a.T_ID,a.M_ID,a.INV_Item,a.INV_UnifyNum,b.M_Name,a.INV_ComName,b.H_Address,b.HTown,b.HZipCode,b.H_Address,b.HCity,b.MTel1,b.EMail,b.INV_CarrierNum,b.T_PayMoney"
                                 + " FROM [HG].[dbo].[MTRAT] a left join [HG].[dbo].[VINVO] b  on a.T_ID=b.T_ID"
                                 + " Where a.T_ID='" + _T_ID + "'";
            ...
            SqlDataAdapter adapter = new SqlDataAdapter();
            adapter.SelectCommand = cmd;
            adapter.Fill(dt);//取出所有資料
                             //string ProductTag = dt.Rows[0]["ProductTag"].ToString();//發票項目別
                             //string getValueByProductTag = "select ditcode,ddesc from MITEM where mitcode = 'PRODUCTTAG' and ditcode ='" + ProductTag + "' and del_tag = '0' order by rank";
                             //SqlCommand getValueByProductTagcmd = new SqlCommand();
                             //getValueByProductTagcmd.CommandText = getValueByProductTag;
                             //getValueByProductTagcmd.Connection = conn;
                             //DataTable dt2 = new DataTable();
                             //SqlDataAdapter adapter_ = new SqlDataAdapter();
                             //adapter_.SelectCommand = cmd;
                             //adapter.Fill(dt2);//取出所有資料
                             //string ItemName_temp = dt2.Rows[0]["ProductTag"].ToString();//發票項目別
            eRPInvoice.Company = dt.Rows[0]["MT_COM"].ToString();
            if (dt.Rows[0]["MT_COM"].ToString() == "HG")
            {
                //eRPInvoice.Company = "華冠投顧股份有限公司";
                eRPInvoice.ItemName = "顧問費";
            }
            if (dt.Rows[0]["MT_COM"].ToString() == "TK")
            {
                eRPInvoice.ItemName = "教育課程";
            }
            var INV_SendState = dt.Rows[0]["INV_SendState"];
            if (INV_SendState.ToString() == "5")
            {
                eRPInvoice.Invoice_item = "4";
            };
            var INV_Item = dt.Rows[0]["INV_Item"];
            if (INV_Item.ToString() == "1")
            {
                eRPInvoice.Invoice_item = "1";
            }
            else if (INV_Item.ToString() == "2")
            {
                eRPInvoice.Invoice_item = "5";
            }
            else
            {
                eRPInvoice.Invoice_item = "1";
            ...
            if (INV_Item.ToString() == "1")
            {
                eRPInvoice.CustomerName = dt.Rows[0]["M_Name"].ToString();
            }
            else if (INV_Item.ToString() == "2")
            {
                eRPInvoice.CustomerName = dt.Rows[0]["INV_ComName"].ToString();
            }



            var T_ID = dt.Rows[0]["T_ID"];
            eRPInvoice.RelateNumber = T_ID.ToString();
            var M_ID = dt.Rows[0]["M_ID"];
            eRPInvoice.CustomerID = M_ID.ToString();
            //var INV_Item = dt.Rows[0]["INV_Item"];
            var INV_UnifyNum = dt.Rows[0]["INV_UnifyNum"];
            if (INV_Item.ToString() == "2")
            {
                eRPInvoice.CustomerIdentifier = INV_UnifyNum.ToString();
            }
            else
            {
                eRPInvoice.CustomerIdentifier = "";
            }
            if (INV_Item.ToString() == "3" && INV_UnifyNum.ToString() != "")
            {
                eRPInvoice.CustomerIdentifier = INV_UnifyNum.ToString();
            }
            else
            ...
            var M_Name = dt.Rows[0]["M_Name"].ToString();
            var INV_ComName = dt.Rows[0]["INV_ComName"].ToString();
            if (INV_Item.ToString() == "1")
            {
                eRPInvoice.CustomerName = M_Name;
            }
            else if (INV_Item.ToString() == "2")
            {
                eRPInvoice.CustomerName = INV_ComName;
            }

            //發票建單失敗：錯誤代碼 0：載具類別(CarruerType)與捐贈單位只能擇一輸入
            var H_Address = dt.Rows[0]["H_Address"];
            var HTown = dt.Rows[0]["HTown"];
            var HZipCode = dt.Rows[0]["HZipCode"];
            var MTel1 = dt.Rows[0]["MTel1"];
            var INV_CarrierNum = dt.Rows[0]["INV_CarrierNum"];
            var T_PayMoney = dt.Rows[0]["T_PayMoney"];
            var HCity = dt.Rows[0]["HCity"];
            var EMail = dt.Rows[0]["EMail"];
            eRPInvoice.CustomerAddr = HCity.ToString() + HTown.ToString() + HZipCode.ToString() + H_Address.ToString();
            eRPInvoice.CustomerPhone = MTel1.ToString();
            eRPInvoice.CustomerEMail = EMail.ToString();
            if (INV_SendState.ToString() == "5")
            {
                eRPInvoice.LoveCode = "55135";
            }
            else
            {
                eRPInvoice.LoveCode = "";
            }
            eRPInvoice.SalesAmount = T_PayMoney.ToString();
            //InvoiceRemark:寄發狀況
            if (INV_SendState.ToString() == "1")
            {
                eRPInvoice.InvoiceRemark = "正常";
                eRPInvoice.LoveCode = "";
            }
            else if (INV_SendState.ToString() == "2")
            {
                eRPInvoice.InvoiceRemark = "空白信封";
                eRPInvoice.LoveCode = "";
            }
            else if (INV_SendState.ToString() == "3")
            {
                eRPInvoice.InvoiceRemark = "發票轉交業務";
                eRPInvoice.LoveCode = "";
            }
            else if (INV_SendState.ToString() == "4")
            {
                eRPInvoice.InvoiceRemark = "不寄";
                eRPInvoice.LoveCode = "";
            }
            else if (INV_SendState.ToString() == "5")
            {
                eRPInvoice.InvoiceRemark = "捐贈";
                eRPInvoice.LoveCode = "55135";
            }
            ///
            eRPInvoice.ItemCount = "1";
            eRPInvoice.ItemWord = "項";
            eRPInvoice.ItemPrice = T_PayMoney.ToString();
            eRPInvoice.ItemAmount = T_PayMoney.ToString();


        }
        return Newtonsoft.Json.JsonConvert.SerializeObject(eRPInvoice);
    }
    ...
    //取得待開發票作業-線上(20221019進度)
    //getOINVO_qyery_01DataList
    public string getOINVO_qyery_01DataList()
    {
        string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
        //log.startWrite("'StartDate;':"+StartDate+"'EndDate;':"+EndDate+"'keyWord;':"+keyWord, className, methodName);

        //設定條件
        List<OINVO_query_01Entity> OINVO_qyery_01DataList = DTRATService.Instance.getOINVO_qyery_01DataList();
        //new List<OINVO_query_01Entity>();//(20221019進度)這裡改寫成用service取回資料

        log.endWrite("'OINVO_qyery_01DataList' :" + OINVO_qyery_01DataList.ToString(), className, methodName);
        var test = Newtonsoft.Json.JsonConvert.SerializeObject(OINVO_qyery_01DataList);
        return Newtonsoft.Json.JsonConvert.SerializeObject(OINVO_qyery_01DataList);
    }
