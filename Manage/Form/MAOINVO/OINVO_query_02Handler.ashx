...
public class MREPO_queryHandler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        {//updateMREPO_queryData
            ...
            //取得特種電子(瑞兔)資料
            case "getOINVO_qyery_02DataList":
                ...
                context.Response.Write(getOINVO_qyery_02DataList());///取得待開發票作業-瑞兔(20221027進度)
                break;
            case "GetItemByT_ID":
                string T_ID_ = context.Request["T_ID"];//要修改發票號碼的員工編號
                context.Response.Write(GetItemByT_ID(T_ID_));
                break;
    ...
    public string GetItemByT_ID(string _T_ID)
    {
        ...
        using (SqlConnection conn = new SqlConnection(WebConfigurationManager.ConnectionStrings[connectionStrings].ToString()))
        {
            ...
            var M_Name = dt.Rows[0]["M_Name"];
            var INV_ComName = dt.Rows[0]["INV_ComName"];
            if (INV_Item.ToString() == "2")
            {
                eRPInvoice.CustomerName = INV_ComName.ToString();
            }
    ...
    public string getOINVO_qyery_02DataList()
    {
        string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
        //log.startWrite("'StartDate;':"+StartDate+"'EndDate;':"+EndDate+"'keyWord;':"+keyWord, className, methodName);

        //設定條件
        List<OINVO_query_02Entity> OINVO_qyery_02DataList = DTRATService.Instance.getOINVO_qyery_02DataList();
        //new List<OINVO_query_01Entity>();//(20221019進度)這裡改寫成用service取回資料

        log.endWrite("'OINVO_qyery_02DataList' :" + OINVO_qyery_02DataList.ToString(), className, methodName);
        var test = Newtonsoft.Json.JsonConvert.SerializeObject(OINVO_qyery_02DataList);
        return Newtonsoft.Json.JsonConvert.SerializeObject(OINVO_qyery_02DataList);
    }
    //此帳號可執行的報表權限
