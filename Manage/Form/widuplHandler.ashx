<%@ WebHandler Language="C#" Class="MSFTFHandler" %>
...
public class MSFTFHandler : IHttpHandler {
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        {
            case "getwiduplData":
                context.Response.Write(getwiduplData(StartDate,EndDate,keyWord));
                break;
            case "delwiduplData":
                id = context.Request["id"];
                context.Response.Write(deletewiduplData(id));
                break;
            case "addwiduplData":
                ...
                context.Response.Write(addwiduplData(ee));
                break;
                //case "updatewiduplData":
                ...
                ////    break;
                ////getMSFTFData
                //case "getMSFTFData":// 專案客戶管理 >個股管理作業
                //    MS_ID = context.Request["MS_ID"];
    ...
    //}
    //getwiduplData
    public string getwiduplData(string StartDate,string EndDate,string keyWord)
    {
        
            string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
        log.startWrite("'StartDate;':"+StartDate+"'EndDate;':"+EndDate+"'keyWord;':"+keyWord, className, methodName);

        List<widupServiceEntity> wSEList = widupService.Instance.getwidupServiceData(StartDate,EndDate,keyWord);//<==========

        log.endWrite("'wSEList' :"+wSEList.ToString(), className, methodName);
        return Newtonsoft.Json.JsonConvert.SerializeObject(wSEList);
    }

    public string deletewiduplData(string MS_ID)
    {
        string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
    ...
    public string addwiduplData(emailEntity ee)
    {
        string methodName = System.Reflection.MethodBase.GetCurrentMethod().Name;
